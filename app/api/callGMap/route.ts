...
const getRateLimitMiddlewares = ({ limit = 30, windowMs = 60 * 1000, delayAfter = 10, delayMs = 100 } = {}) => [
  slowDown({ keyGenerator: getIP, windowMs, delayAfter, delayMs: () => delayMs }),
  rateLimit({ keyGenerator: getIP, windowMs, max: limit }),
]

// eslint-disable-next-line @typescript-eslint/ban-types
const applyMiddleware = (middleware: Function) => (req: NextRequest) =>
  new Promise((resolve, reject) => {
    middleware(req, NextResponse, (result: unknown) => (result instanceof Error ? reject(result) : resolve(result)))
  })
...
export async function POST(req: NextRequest) {
  ...
  if (process.env.LIMIT_API) {
    ...
    try {
      await Promise.all(middlewares.map((middleware) => middleware(req)))
    } catch {
      return NextResponse.json('Too Many Requests', { status: 420 })
    }
  }
