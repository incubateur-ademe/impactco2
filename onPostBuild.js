...
module.exports = exports.onPostBuild = async ({ graphql }) => {
  const { data } = await graphql(`
    query {
      ...
      docsHandbook: allMdx(
        filter: { fields: { slug: { regex: "/^/handbook|^/docs/" } } }
      ) {
        nodes {
          fields {
            slug
  ...
  const res = await fetch(
    'https://d27nj4tzr3d5tm.cloudfront.net/Website-Assets/Fonts/Matter/MatterSQVF.woff',
    {
      headers: {
  ...
  await new Promise((resolve, reject) => {
    const fileStream = fs.createWriteStream(
      path.resolve(__dirname, '../fonts/matter.woff')
    )
  ...
  for (const post of data.blog.nodes) {
    ...
    const image = fs.readFileSync(
      path.resolve(__dirname, featuredImage.absolutePath),
      {
        encoding: 'base64',
      }
    )
    await createOG({
      html: blogTemplate({
        title,
        authorData: authorData && authorData[0],
        image,
        font,
      }),
  ...
  for (const post of data.docsHandbook.nodes) {
    ...
    if (!title || !timeToRead || !excerpt || !lastUpdated || !post.contributors)
      continue
    ...
    await createOG({
      html: docsHandbookTemplate({
        ...
        contributors,
        breadcrumbs: [
          { name: fields.slug.startsWith('/docs') ? 'Docs' : 'Handbook' },
          ...(breadcrumbs || []),
        ],
      }),
      slug: fields.slug,
  ...
  for (const post of data.customers.nodes) {
    ...
    const logoType = frontmatter.logo.absolutePath.includes('.svg')
      ? 'svg+xml'
      : 'image/jpeg'
    const featuredImage = fs.readFileSync(
      path.resolve(__dirname, frontmatter.featuredImage.absolutePath),
      {
        encoding: 'base64',
      }
    )
    const logo = fs.readFileSync(
      path.resolve(__dirname, frontmatter.logo.absolutePath),
      {
        encoding: 'base64',
      }
    )
