...
describe("YearlyLivraison - afficher le bilan carbone selon la fréquence d'achat", () => {
  ...
  test('Double le bilan carbone si on double le nombre de colis', async () => {
    ...
    // Then
    expect(await (screen.queryByTestId('numbers') as HTMLInputElement).value).toBe('2')
    expect(await (screen.queryByTestId('frequences') as HTMLInputElement).value).toBe('par_an')
  ...
  test('Multiplie par 12 le bilan carbone si on passe à une fréquence par mois', async () => {
    ...
    // Then
    expect(await (screen.queryByTestId('numbers') as HTMLInputElement).value).toBe('1')
    expect(await (screen.queryByTestId('frequences') as HTMLInputElement).value).toBe('par_mois')
    expect(await (screen.queryByTestId('kgCo2e') as HTMLInputElement)).toHaveTextContent('25,60 kg CO2e')
  })
  test('Multiplie par 52 le bilan carbone si on passe à une fréquence par semaine', async () => {
    ...
    // Then
    expect(await (screen.queryByTestId('numbers') as HTMLInputElement).value).toBe('1')
    expect(await (screen.queryByTestId('frequences') as HTMLInputElement).value).toBe('par_semaine')
