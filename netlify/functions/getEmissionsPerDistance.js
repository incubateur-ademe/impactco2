...
exports.handler = async function (event) {
  matomo.track(`https://api.impactco2.fr/beta/getEmissionsPerDistance?km=${event.queryStringParameters.km}`);
  ...
  const filter = event.queryStringParameters.filter || (event.queryStringParameters.transportations ? "all" : "smart");
  const activeTransportations = event.queryStringParameters.transportations
    ? event.queryStringParameters.transportations.split(",").map((id) => Number(id))
    : [];

  const ignoreRadiativeForcing = event.queryStringParameters.ignoreRadiativeForcing || false;
