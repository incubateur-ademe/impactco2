...
const AddNpsNoteValidation = z.object({
  ...
  itineraireStart: z.string().optional(),
  itineraireEnd: z.string().optional(),
})
...
export const addNpsNote = async ({
  ...
  source: string
  tracking: string
  itineraireStart?: string
  itineraireEnd?: string
}) => {
  const dbId = process.env.NOTION_API_DB_NPS
  const parsed = AddNpsNoteValidation.safeParse({ note, source, tracking, itineraireStart, itineraireEnd })
