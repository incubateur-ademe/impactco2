...
const FaqsOverscreen = ({ filter, page }: { filter: string } & Omit<FAQSListProps, 'faqs'>) => {
  const { setOverscreen, language } = useParamContext()
  const t = useTranslations('faq')
  const tModal = useTranslations('modal')
  const [search, setSearch] = useState('')
  const [faqs, setFaqs] = useState<FAQType[]>([])
  const { faqsByCategory } = useSearchableFAQs({ faqs, search, categoryOrder: faqCategoriesOrder })
    ...
    <>
      <div className={headerStyles.header}>
        <p className={styles.searchExemples}>
          {t('exemple')}{' '}
          <Button onClick={() => setSearch(t('exemple-1'))} asLink>
            {t('exemple-1')}
          </Button>
          ,{' '}
          <Button onClick={() => setSearch(t('exemple-2'))} asLink>
            {t('exemple-2')}
          </Button>{' '}
          {t('or')}{' '}
          <Button onClick={() => setSearch(t('exemple-3'))} asLink>
            {t('exemple-3')}
          </Button>
        </p>
