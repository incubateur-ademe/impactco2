...
export const distanceComparisonTest = async (page: Page | FrameLocator, prod?: boolean) => {
  ...
  await expect(page.getByTestId('comparison-tile-1')).toHaveText('Avion court courrier2.25 kg CO₂e Modifier')

  await page.getByTestId('input-km-value').fill('10000')
  await expect(page.getByTestId('comparison-tile-0')).toHaveText(
    'TGV29.3 kg CO₂eMoyen le plus écologique1,639Kg CO₂eévités Modifier'
  )
  await expect(page.getByTestId('comparison-tile-1')).toHaveText('Avion long courrier1,668 kg CO₂e Modifier')
}
