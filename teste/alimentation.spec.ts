...
test('Alimentation simulateur params', async ({ page }) => {
  ...
  await expect(page.getByRole('button', { name: 'Aller plus loin' }).nth(0)).toBeVisible()
  ...
  await expect(page.getByRole('button', { name: 'Aller plus loin' }).nth(0)).not.toBeVisible()
