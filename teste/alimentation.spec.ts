...
test('Alimentation simulateur', async ({ page }) => {
  ...
  await expect(page.getByTestId('clipboard-box')).toHaveText(
    '<script data-name="impact-co2" src="http://localhost:3000/iframe.js" data-type="/alimentation" data-search="?alimentationCategory=rayon&theme=default&language=fr"></script>'
  )
  ...
  await expect(page.getByTestId('clipboard-box')).toHaveText(
    '<script data-name="impact-co2" src="http://localhost:3000/iframe.js" data-type="/alimentation" data-search="?alimentationEquivalents=boeuf,porc,crevettes,cabillaud,lieu,dorade,saumon,moules,huitres,thon,sardines&theme=default&language=fr"></script>'
  )
...
test('Alimentation simulateur params', async ({ page }) => {
  ...
  await expect(page.getByTestId('alimentation-category-boucherie')).not.toBeVisible()
  await expect(page.getByTestId('category-boeuf')).toHaveText('Boeuf28 kg CO₂e')
  await expect(page.getByRole('button', { name: 'Questions fréquentes' }).nth(0)).toBeVisible()
  ...
  await expect(page.getByTestId('category-boeuf')).toHaveText('Boeuf28 kg CO₂e')
  await expect(page.getByTestId('category-porc')).toHaveText('Porc6.67 kg CO₂e')
  await expect(page.getByRole('button', { name: 'Questions fréquentes' }).nth(0)).not.toBeVisible()
