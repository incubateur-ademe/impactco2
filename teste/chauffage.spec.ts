...
test('Simulator chauffage', async ({ page }) => {
  ...
  await expect(page.getByTestId('clipboard-box')).toHaveText(
    '<script data-name="impact-co2" src="http://localhost:3000/iframe.js" data-type="chauffage" data-search="?m2=200&language=fr&theme=default"></script>'
  )

  await page.getByTestId('custom-param-theme-select').selectOption('night')
  await expect(page.getByTestId('clipboard-box')).toHaveText(
    '<script data-name="impact-co2" src="http://localhost:3000/iframe.js" data-type="chauffage" data-search="?m2=200&language=fr&theme=night"></script>'
  )

  await expect(page.getByTestId('input-m2')).toHaveValue('200')
  await page.getByTestId('input-m2-value').fill('300')
  await expect(page.getByTestId('clipboard-box')).toHaveText(
    '<script data-name="impact-co2" src="http://localhost:3000/iframe.js" data-type="chauffage" data-search="?m2=300&language=fr&theme=night"></script>'
  )
  await expect(page.getByTestId('category-link')).toHaveCount(9)
