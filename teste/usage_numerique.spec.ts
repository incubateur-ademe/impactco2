...
test('Update share and integrate values when modifying parameters', async ({ page }) => {
  await expect(page.getByTestId('usagenumerique-generated-value')).toContainText('0.28')
  await expect(page.getByTestId('etiquette-voiturethermique-value')).toContainText('65.8')
  await expect(page.getByTestId('etiquette-repasavecduboeuf-value')).toContainText('1.97')
  await expect(page.getByTestId('etiquette-streamingvideo-value')).toContainText('224')
  ...
  await expect(page.getByTestId('usagenumerique-generated-value')).toContainText('1.9')
...
test('Load default values', async ({ page }) => {
  ...
  await expect(page.getByTestId('usagenumerique-generated-value')).toContainText('1.94')
})
