...
test('Transport distance comparison', async ({ page }) => {
  ...
  await page.getByTestId('header-integrate-button').click()
  await page.getByTestId('text-select-comparison-1').selectOption('tgv')
  await page.getByTestId('text-select-comparison-2').selectOption('avion')
  await page.getByTestId('cancel-button').click()

  await expect(page.getByTestId('comparison-tile-0')).toContainText('2.56Kg CO₂eévitésTGV0.03 kg CO₂e Modifier')
...
test('Transport distance default values', async ({ page }) => {
  ...
  await expect(page.getByRole('link', { name: 'Bus électrique 0.22 kg CO₂e' })).not.toBeVisible()
  await page.getByRole('button', { name: 'Voir tous les modes de' }).click()
  await expect(page.getByRole('link', { name: 'Bus électrique 0.22 kg CO₂e' })).toBeVisible()

  await page.goto(
    'http://localhost:3000/outils/transport?comparison=scooter,avion&km=15&itineraireStart=Lyon%20France&itineraireEnd=Paris%20France&defaultMode=comparison&modes=intercites,voiturethermique,voitureelectrique,velo,veloelectrique,busthermique,tramway,metro,scooter,moto,rer,ter,trottinette,busgnv'
  )
  ...
  await page.goto('http://localhost:3000/outils/transport?&tabs=distance&km=15&mode=comparison&comparison=metro,avion')
